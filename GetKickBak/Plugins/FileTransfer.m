...
- (void) upload:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options {
   ...
   if(errorCode > 0) {
      result = [CDVPluginResult resultWithStatus: CDVCommandStatus_OK messageAsInt: INVALID_URL_ERR cast: @"navigator.jffileTransfer._castTransferError"];
   ...
   //[req setValue:[[NSNumber numberWithInteger:[postBody length]] stringValue] forHTTPHeaderField:@"Content-Length"];
	...
	JFFileTransferDelegate* delegate = [[JFFileTransferDelegate alloc] init];
...
@implementation JFFileTransferDelegate
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection 
{
   ...
   CDVPluginResult* result = [CDVPluginResult resultWithStatus: CDVCommandStatus_OK messageAsDictionary: uploadResult cast: @"navigator.jffileTransfer._castUploadResult"];
   [command writeJavascript:[result toSuccessCallbackString: callbackId]];
}

- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error 
{
   CDVPluginResult* result = [CDVPluginResult resultWithStatus: CDVCommandStatus_OK messageAsInt: CONNECTION_ERR cast: @"navigator.jffileTransfer._castTransferError"];
...
{
   NSMutableDictionary* uploadResult = [NSMutableDictionary dictionaryWithCapacity:2];
   [uploadResult setObject:[NSNumber numberWithInt: totalBytesWritten] forKey:@"progress"];
   [uploadResult setObject:[NSNumber numberWithInt: totalBytesExpectedToWrite] forKey:@"total"];
   CDVPluginResult* result = [CDVPluginResult resultWithStatus: CDVCommandStatus_OK messageAsDictionary: uploadResult cast: @"navigator.jffileTransfer._updateProgress"];
   NSLog(@"Sent %@ of %@ bytes", [NSNumber numberWithInt: totalBytesWritten], [NSNumber numberWithInt: totalBytesExpectedToWrite]);
   [command writeJavascript:[result toSuccessCallbackString: callbackId]];
